...
var publish_topic_path = {
    ...
    temphumbaro: 'events/temperature_sensor/',
...
// Handle temperature/humidity events
rfxtrx.on("temperaturehumidity1", function (evt) {
    publishEvent('temperature', evt);
});

// Handle temperature/humidity/barometric pressure events
rfxtrx.on("temphumbaro1", function (evt) {
    publishEvent('temphumbaro', evt);
});

// Handle power consumption events
rfxtrx.on("elec23", function (evt) {
...
function publishEvent(type, evt) {
    ...
    } else if (type === 'temphumbaro') {    
        message = {
            temperature: evt.temperature.toString(),
            humidity: evt.humidity.toString(),
            pressure: evt.barometer.toString(),
            forecast: evt.forecast.toString(),
            battery: evt.batteryLevel.toString(),
            signal: evt.rssi.toString(),
            timestamp: hautil.toIsoString(new Date())
        };
    ...
    if (evt.count === 0) {
      message.totalWatts = evt.energy.toString();
      delete message.currentWatts;
      ha.publish(publish_topic_path[type] + sensor + '/consumption', message, { retain: true });
    }
