...
/* Variables */
var subscribe_topics = ['events/temperature_sensor/#', 'events/power_meter/#'];
...
Driver.prototype.run = function () {
    ...
    ha.on('message', function (topic, message) {
        ...
        if (topicsplit[1] == 'temperature_sensor') {
            updateCache('weather', topicsplit[2], 'temperature', message.temperature);
            updateCache('weather', topicsplit[2], 'humidity', message.humidity);
        ...
        if (topicsplit[1] == 'power_meter') {
            if (typeof message.currentWatts !== 'undefined' && message.currentWatts !== 0) { 
                ...
                updateCache('power', topicsplit[2], 'currentWatts', message.currentWatts, 2);
                ...
                updateCache('power', topicsplit[2], 'currentWatts', message.Power, 2);
 ...
 */
function updateCache(type, device, measurement, value, min_change) {
    // Initialise min_change to zero if not set
    min_change += 0;
    ...
    // Only record the measurement if the value has changed and it's one we want to trigger a save
    if (latestReading[type][device][measurement] !== value) {

        if (Math.abs(latestReading[type][device][measurement] - value) < min_change) {
          ha.info(device + ":" + measurement + ": " + latestReading[type][device][measurement] + " -> " + value + " (skipped, too small)");
          return;
        }
