...
Driver.prototype.run = function () {
    ...
    ha.on('message', function (topic, message) {
        ...
        if (topicsplit[1] == 'power_meter') {
            if (message.currentWatts !== 0) { 
                updateCache('power', 'main', 'currentWatts', message.currentWatts);
                updateCache('power', 'main', 'timestamp', message.timestamp);
            }
            if (message.totalWatts !== undefined && message.totalWatts !== 0) {
               updateCache('power', 'main', 'totalWatts', message.totalWatts);
               updateCache('power', 'main', 'timestamp', message.timestamp);
            }
        }
    });
        ...
        Object.keys(latestReading[key]).forEach(function (key) {
            ...
            if (item.state === 'dirty') {
                ...
                if (type == 'weather') {
                    ...
                    data = {
                        datetime: item.timestamp,
                        energy: item.totalWatts,
                        current: item.currentWatts / 230,
                    };
                } else {
                    return;
                }
 ...
 */
function updateCache(type, device, measurement, value) {
