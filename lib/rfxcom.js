...
Driver.prototype.run = function () {
  ...
  rfxtrx.on('ready', function () {
    ...
    watchdog = new Watchdog(60 * 1000)
    watchdog.on('timeout', function () {
      ha.error('No activity for 60 seconds, reconnecting to rfxcom')
      rfxtrx.close()
      rfxtrx.initialise(function (err) {
        ha.info('Reconnected')
      })
    })
  })
  ...
  rfxtrx.on('disconnect', function () {
    ha.log('error', 'Serial port disconnected')
    ha.close()
    process.exit()
  })
